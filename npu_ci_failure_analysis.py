...
"""
import argparse, subprocess, json, re, os, gzip, sys, tempfile, base64
...
# ---------- Step 3: 失败 run → 定位 NPU job（含门禁 fallback） ----------
...
BUCKETS = [
    (r'FAILED:\s*\[code=1\]|\berror: no viable|\berror:.*(?:expected|undeclared|cannot convert|no member named)|'
     r'error:.*required.*include|\bclang\+\+.*error:|Error:.*CMake Error', "编译失败(C++/MLIR)"),
    (r'NPU function error|aclnn\w*\s*failed|error code is \d+', "昇腾算子执行错误(ACL)"),
    (r'cannot open shared object file|\.so: cannot open|torch_extensions.*\.so', "自定义算子so缺失(csrc构建)"),
    (r'(?:SIGKILL|exit code 137|killed.*(?:OOM|137)|OOMKilled|Signal 9|container.*not found)', "进程被kill(OOM/超内存)"),
    # 分布式桶必须收紧：e2e 训练日志满屏 [Rank N] 前缀，裸 rank.*(fail|error) 会误匹配
    (r'HCCL\w*(error|timeout|failed)|Connection reset|broken pipe|RayTaskError|ray\.exceptions|CollectiveError', "分布式通信/HCCL/Ray"),
    (r'HfHubHTTPError|huggingface_hub|Failed to download metadata|repomd\.xml|Cannot download|Curl error|503.*Service Unavailable', "模型/包下载失败(网络)"),
    (r'timed out|TimeoutError|UV_HTTP_TIMEOUT|timeout.*exceed|timed out waiting', "超时"),
    (r'out of memory|OOM error|MemoryError|memory.*not enough|aclrtMalloc failed|alloc.*failed.*memory', "OOM/显存不足"),
    (r'No space left|disk full|ENOSPC', "磁盘不足"),
    (r'ImportError|ModuleNotFoundError|No module named', "依赖/安装(ImportError)"),
    (r'AssertionError|E\s+assert', "断言失败(代码或精度)"),
    (r'ShellCheck|shellcheck', "静态检查(ShellCheck)"),
    (r'AttributeError|TypeError|ValueError|KeyError|IndexError', "Python运行时错误"),
    (r'Either .tests. or .config_file_path. must be provided|must be provided', "测试参数缺失(config未传入)"),
]
...
for f, run_id, job_id, job_name, is_npu in failed_jobs:
    ...
    detail.append((f, job_name[:32], tag, bucket, sig[:70]))
print(f"\n=== Step4 已分类日志 {logs_done} 份（[NPU]=NPU job / [gate]=CPU门禁fallback，附证据片段）===")
for d in detail:
    print(f"  [{d[3][:4]:4s}|{d[2]:4s}] {d[0][:26]:26s} {d[1]:30s} → {d[3]:16s} | {d[4]}")
